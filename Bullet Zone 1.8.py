...
def main():
    ...
    while True:
        ...
        if True: #sp tickers
            ...
            if sp3_p2 <550:
                sp3_p2 += 1
        if P2_AI and (timer%2 == 0): #AI p2
            if timer == 2: #start conditions
                AIx_timer = 0
                AIy_timer = 0
                activeAI = 0
            if (hp_spot != [-100,-100]) and (activeAI > 0): #hp ai
                if hp_spot[0] > player2.x:   
                    player2_Xspeed += player_acceleration
                    if player2_Xspeed < 0:
                        player2_Xspeed += player_acceleration
                elif hp_spot[0] < player2.x:   
                    player2_Xspeed -= player_acceleration
                    if player2_Xspeed > 0:
                        player2_Xspeed -= player_acceleration
                if hp_spot[1] > player2.y:   
                    player2_Yspeed += player_acceleration
                    if player2_Yspeed < 0:
                        player2_Yspeed += player_acceleration
                elif hp_spot[1] < player2.y:   
                    player2_Yspeed -= player_acceleration
                    if player2_Yspeed > 0:
                        player2_Yspeed -= player_acceleration
            elif activeAI > 0:
                activeAI -= 1
                pygame.draw.rect(WIN,CYAN, pygame.Rect(WIDTH - 150, 100, 100,100))
                
                #track player1 using velocity


                if activeAI == 1: #end active AI
                    activeAI = 0
                    AIx_timer = 0
                    AIy_timer = 1
            else: #random ai
                pygame.draw.rect(WIN,GREEN, pygame.Rect(WIDTH - 150, 100, 100,100))
                if (AIx_timer<6) and (AIy_timer<6): #turn on active AI
                    activeAI = 101
                    AIx_timer = 0
                    AIy_timer = 0
                if (AIx_timer == 0):
                    AIx_timer = dice(5,8)
                    AIx_dir = random.randint(-1,1)
                if (AIy_timer == 0):
                    AIy_timer = dice(5,8)
                    AIy_dir = random.randint(-1,1)
                if AIx_timer > 0:
                    AIx_timer-=1
                    player2_Xspeed += AIx_dir * .5 * player_acceleration
                    if ((player2_Xspeed > 0) and (AIx_dir < 0)) or ((player2_Xspeed < 0) and (AIx_dir > 0)):
                        player2_Xspeed += AIx_dir * player_acceleration
                if AIy_timer > 0:
                    AIy_timer-=1
                    player2_Yspeed += AIy_dir * .5 * player_acceleration
                    if ((player2_Yspeed > 0) and (AIy_dir < 0)) or ((player2_Yspeed < 0) and (AIy_dir > 0)):
                        player2_Yspeed += AIy_dir * player_acceleration

            if sp1_p2 == 450: #sp1 p2
                SP_P2_active = 0
                ...
                sp1_p2 = 0
            if sp2_p2 == 600: #sp2 p2
                SP_P2_active = -45
                sp2_p2_active = 35
                sp2_p2 = 0
            if sp3_p2 == 550: #sp3 p3   
                SP_P2_active = 0
        else: #live p2
            ...
            if keys_pressed[pygame.K_DOWN]: #down acceleration
                ...
                if player2_Yspeed < 0:
                    player2_Yspeed += player_acceleration
            if keys_pressed[pygame.K_RALT]: #stop
                player2_Xspeed = 0
                player2_Yspeed = 0
            for event in pygame.event.get(): #special attacks
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_k and sp2_p2 > 500: #sp2 p2
                        ...
                        sp2_p2_active = 35
                        sp2_p2 = 0
                    if event.key == pygame.K_i and sp3_p2 > 450: #sp3 p3   
                        SP_P2_active = 0
        for event in pygame.event.get(): #special attacks
            if event.type == pygame.QUIT:
                    pygame.quit()
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_TAB and sp2_p1 == 500: #sp2 p1
                    ...
                    sp2_p1_active = 35
                    sp2_p1 = 0
                if event.key == pygame.K_BACKQUOTE and sp3_p1 == 450: #sp3 p1
                    SP_P1_active = 0
